...
is done by Claude. A small helper script (`scripts/notes.py`) handles the
deterministic, testable parts: resolving the notes directory, sanitizing titles
into safe filenames, listing untitled daily notes, and renaming files (using
`git mv` when the vault is a Git repo, so history is preserved). Renames and
new files never overwrite an existing file — a numeric suffix is added on
collision.
