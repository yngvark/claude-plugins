...
1. **Path allowlist** — only `repos/{owner}/{repo}/{resource}` paths, `orgs/{org}/properties/{schema,values}` paths, `search/{type}` paths, and `notifications` / `notifications/threads/{id}` paths are permitted, where each segment is one of a fixed set. Checked via string splitting, no regex; query strings and fragments are stripped before validation.
